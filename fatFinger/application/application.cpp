...
bool application::CheckToLock(TThostFtdcInstrumentIDType InstrumentID,
  ...
  for (int i = 0; i < tradeVector.size(); i++) {
    if (tradeVector[i]->Direction == THOST_FTDC_D_Buy &&
        tradeVector[i]->OffsetFlag == THOST_FTDC_OF_Open) {
      buy = buy + tradeVector[i]->Volume;
    } else if (tradeVector[i]->Direction == THOST_FTDC_D_Sell &&
               tradeVector[i]->OffsetFlag == THOST_FTDC_OF_Open) {
