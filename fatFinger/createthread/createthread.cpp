...
createThread::createThread(pthread_cond_t *cond,
...
int createThread::threadRun() {
  ...
  while (1) {
    ...
    CThostFtdcDepthMarketDataField *pCurrentDepthMarketData =
        pmdspi->getDPMarketDataField(InstrumentID);
    ...
    if (CurrentBidPrice >= (BidPrice * (1 + THRESHOLD / 100))) {
      ...
      cout << "----------------" << InstrumentID << endl;
      APPLICATION->ReqOrderInsert(InstrumentID, THOST_FTDC_D_Sell,
                                  CurrentBidPrice, MAXCOUNT,
                                  THOST_FTDC_OF_Open);
      ...
      APPLICATION->ReqOrderInsert(InstrumentID, THOST_FTDC_D_Buy,
                                  CurrentAskPrice, MAXCOUNT,
                                  THOST_FTDC_OF_Open);
    } else {
      //说明以前还不是乌龙指
      if (!FindFatFinger) {
      ...
      else if (CurrentLastPrice <= (LastPrice * (1 + NORMALTHRESHOLD / 100)) &&
               CurrentLastPrice >= (LastPrice * (1 - NORMALTHRESHOLD / 100))) {
        BidPrice = CurrentBidPrice;
        AskPrice = CurrentAskPrice;
        LastPrice = CurrentLastPrice;
